...
import { Search, Users, ArrowLeft, BarChart3 } from "lucide-react";
          ...
          <div className="flex flex-wrap gap-3">
            {Object.entries(stats.language_distribution).map(([lang, count]) => (
              <div key={lang} className="flex items-center gap-2 bg-muted/50 rounded-lg px-3 py-2">
                <Badge variant="outline" className="text-xs">{LANGUAGE_LABELS[lang as Language] ?? lang}</Badge>
            ...
            {filtered.map((r, i) => (
              <tr key={r.msisdn} className="border-b last:border-b-0 hover:bg-accent/30 transition-colors">
                ...
                <td className="px-5 py-2.5">
                  <Badge variant="outline" className="text-xs">{LANGUAGE_LABELS[r.lang as Language] ?? r.lang}</Badge>
                </td>
              </tr>
            ))}
          </tbody>
